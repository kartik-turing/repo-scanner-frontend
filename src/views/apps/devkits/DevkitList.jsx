...
// Component Imports
import AddCustomerDrawer from './AddDevkitDrawer'
...
import AddUniverseDrawer from './AddDevkitDrawer'
import { selectLoader } from '@/utils/Helpers'
import './../../apps/logistics/dashboard/styles.css'
import AddDevkitDrawer from './AddDevkitDrawer'
...
const DevkitList = ({ initialData, loading, onDataUpdated }) => {
  // States
  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.accessor('supportedCompliance', {
        header: 'Supported Compliance',
        cell: info => (
          <div className='flex flex-wrap gap-1'>
            {info.getValue()?.map((compliance, index) => (
              <Chip key={index} label={compliance} size='small' />
            ))}
          </div>
        )
      }),
      columnHelper.accessor('downloadUrl', {
        ...
        cell: info => (
          <Link href={info.getValue()} target='_blank'>
            <Typography className='text-primary underline'>Download</Typography>
          </Link>
        )
      }),
      columnHelper.accessor('documentationUrl', {
        header: 'Documentation URL',
        cell: info => (
          <Link href={info.getValue()} target='_blank'>
            <Typography className='text-primary underline'>Docs</Typography>
          </Link>
        )
      }),
  ...
  const deleteItem = async itemId => {
    ...
    try {
      setDeleting(true)
      const response = await fetch(process.env.NEXT_PUBLIC_API_URL + `api/devkits/${itemId}`, {
        method: 'DELETE',
        headers: {
