...
import AddScanSessionDrawer from './AddScanSessionDrawer'
...
const ScanSessionList = ({ initialData, loading, onDataUpdated }) => {
  // States
  ...
  const columns = useMemo(
    () => [
      ...
      // columnHelper.accessor('schedulerId', {
      //   header: 'Scheduler Id',
      //   cell: info => <Typography>{info.getValue() || 'N/A'}</Typography>
      // }),
      ...
      columnHelper.accessor('status', {
        ...
        cell: info => {
          ...
          const formattedStatus = statusMapping[status] || status // Default to the status value if not in mapping
          return <Chip label={formattedStatus} color='primary' size='small' />
        }
      }),
  ...
  const deleteItem = async itemId => {
    ...
    try {
      setDeleting(true)
      const response = await fetch(process.env.NEXT_PUBLIC_API_URL + `api/scan/${itemId}`, {
        method: 'DELETE',
        headers: {
...
export default ScanSessionList
