...
import AddDiscoveryNetworkDrawer from './AddDiscoveryNetworkDrawer'
...
const DiscoveryNetworkList = ({ initialData, loading, onDataUpdated }) => {
  // States
  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.accessor('scanSessionId', {
        header: 'Scan Session ID',
        cell: info => <Typography>{info.getValue() ?? 'N/A'}</Typography>
      }),
      columnHelper.accessor('discoveryType', {
      ...
      columnHelper.accessor('cryptographyComment', {
        header: 'Cryptography Comment',
        cell: info => <Typography>{info.getValue()}</Typography>
      }),
      columnHelper.accessor('quantumVulnerable', {
        header: 'Quantum Vulnerable',
        cell: info => (
          <Chip label={info.getValue() ? 'Yes' : 'No'} color={info.getValue() ? 'error' : 'success'} size='small' />
        )
      }),
      columnHelper.accessor('vulnerableAlgorithms', {
        header: 'Vulnerable Algorithms',
        cell: info => <Typography>{info.getValue()}</Typography>
      }),
      columnHelper.accessor('pqcSchemePresent', {
        header: 'PQC Scheme Present',
        cell: info => (
          <Chip label={info.getValue() ? 'Yes' : 'No'} color={info.getValue() ? 'primary' : 'default'} size='small' />
        )
      }),
      columnHelper.accessor('pqcAlgorithms', {
        header: 'PQC Algorithms',
        cell: info => <Typography>{info.getValue()}</Typography>
      }),
  ...
  const deleteItem = async itemId => {
    ...
    try {
      setDeleting(true)
      const response = await fetch(process.env.NEXT_PUBLIC_API_URL + `api/discovery-list-network/${itemId}`, {
        method: 'DELETE',
        headers: {
...
export default DiscoveryNetworkList
