...
const UserListTable = ({ initialData, loading, onDataUpdated }) => {
  // States
  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.accessor('passwordHash', {
        ...
        cell: info => <Typography sx={{ fontSize: '0.75rem', wordBreak: 'break-all' }}>{info.getValue()}</Typography>
      }),
      columnHelper.accessor('role', {
        header: 'User role',
        cell: info => <div className='flex flex-wrap gap-1'>{<Chip label={info.getValue()} size='small' />}</div>
      }),
      // columnHelper.accessor('customer_id', {
      //   header: 'Linked to customers table',
      //   cell: info => <Typography>{info.getValue() || '-'}</Typography>
      // }),
      // columnHelper.accessor('partner_id', {
      //   header: 'Linked to partners table',
      //   cell: info => <Typography>{info.getValue() || '-'}</Typography>
      // }),
  ...
  const deleteItem = async itemId => {
    ...
    try {
      setDeleting(true)
      const response = await fetch(process.env.NEXT_PUBLIC_API_URL + `api/users/${itemId}`, {
        method: 'DELETE',
        headers: {
